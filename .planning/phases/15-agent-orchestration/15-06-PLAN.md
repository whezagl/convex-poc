...
must_haves:
  ...
  artifacts:
    - path: "convex/tasks.ts"
      provides: "Convex mutations for task CRUD operations"
      exports: ["create", "updateStatus", "updateClassification", "listByStatus"]
    - path: "convex/subtasks.ts"
      provides: "Convex mutations for sub-task CRUD operations"
      exports: ["create", "updateProgress", "listByTask"]
  ...
  key_links:
    ...
    - from: "SubTaskManager.ts"
      to: "convex/tasks.ts"
      via: "convex.mutations.tasks.updateStatus"
      pattern: "convex\\.mutations\\.tasks"
    - from: "SubTaskManager.ts"
      to: "convex/subtasks.ts"
      via: "convex.mutations.subtasks.create"
      pattern: "convex\\.mutations\\.subtasks"
---
...
<task type="auto">
  ...
  <files>convex/tasks.ts, convex/subtasks.ts, convex/schema.ts</files>
  <action>
    ...
    Update convex/schema.ts to add classification fields and indexes:
    ```typescript
    tasks: defineTable({
      title: v.string(),
      description: v.optional(v.string()),
    ...
    Note: The updateClassification mutation is created here (not in 15-05) because
    it requires the classification fields to exist in the schema first.
  </action>
  <verify>grep -q "export const create\|export const updateStatus\|export const updateClassification" convex/tasks.ts && grep -q "export const create\|export const updateProgress" convex/subtasks.ts && grep -q "classification:" convex/schema.ts</verify>
  <done>Convex mutations added for task creation, status updates, classification storage, and sub-task management with proper schema fields and indexes</done>
</task>
