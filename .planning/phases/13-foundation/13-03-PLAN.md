...
must_haves:
  truths:
    ...
    - "Schema is deployed to self-hosted Convex backend"
  ...
  key_links:
    - from: "convex/schema.ts"
      ...
      via: "import { TaskStatus, TaskPriority } from '@convex-poc/shared-types/task'"
...
<task type="auto">
  ...
  <verify>
    - Run `npx convex dev` to start local development backend
  ...
  <done>
    Convex schema is extended with tasks and subtasks collections and deployed
  </done>
</task>
...
<task type="auto">
  ...
  <done>
    Task CRUD functions are created and deployable to Convex backend
  </done>
</task>
...
2. Schema is deployed to self-hosted Convex backend
3. Indexes defined for efficient Kanban queries (status, priority, createdAt)
4. Task and SubTask CRUD functions are deployed
5. Existing agentSessions and workflows collections preserved
</success_criteria>
