...
/**
 * Configuration options specific to GLMPlannerAgent.
 *
 * @property detailLevel - Level of detail in generated plans (default: "basic")
 */
export interface GLMPlannerConfig extends AgentConfig {
  detailLevel?: "basic" | "detailed";
}

/**
 * GLMPlannerAgent specializes in task decomposition and planning using GLM-4.7.
 *
 * This agent extends GLMBaseAgent to analyze input tasks and break them down
 ...
 * Example usage:
 * ```typescript
 * const planner = new GLMPlannerAgent({ agentType: "glm-planner" });
 ...
 * ```
 */
export class GLMPlannerAgent extends GLMBaseAgent {
  protected readonly detailLevel: "basic" | "detailed";

  /**
   * Creates a new GLMPlannerAgent instance.
   *
   * @param config - Agent configuration with optional planner-specific settings
   * @throws Error if ZAI_API_KEY is not set
   */
  constructor(config: GLMPlannerConfig) {
    super({
      agentType: config.agentType || "glm-planner",
      model: config.model,
...
Remember: Output ONLY valid JSON, nothing else.`;
   ...
   * the JSON response from GLM-4.7, returning a typed PlanResult.
   *
  ...
  public async executeWithWorkflow(
    ...
    const agentWithWorkflow = new GLMPlannerAgent({
